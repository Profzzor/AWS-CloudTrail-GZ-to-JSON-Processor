...
def unzip_gz_to_json(gz_file_path, output_dir):
    ...
    try:
        with gzip.open(gz_file_path, 'rt', encoding='utf-8') as gz_file:
            ...
            json_file_name = os.path.splitext(os.path.splitext(base_name)[0])[0] + '.json'
...
def combine_json_files(input_dir, output_json_file):
    """Combines JSON contents from all files. If a file contains 'Records', use them; else include the entire JSON."""
    combined_data = []
                ...
                try:
                    with open(json_file_path, 'r', encoding='utf-8') as json_file:
                        ...
                        if isinstance(data, dict) and "Records" in data and isinstance(data["Records"], list):
                            combined_data.extend(data["Records"])
                        else:
                            # Include the whole JSON content (dict or list)
